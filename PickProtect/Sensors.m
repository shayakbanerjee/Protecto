...
-(float) calcQy:(NSData *)data {
    ...
    UInt16 qy_read;
    qy_read = (scratchVal[1] & 0xff) | ((scratchVal[0] << 8) & 0xff00);
    float qy =(float)qy_read/QUATERNION_RANGE;
    if(qy >=2) qy-=4;
    return qy;
}
-(float) calcQz:(NSData *)data {
    ...
    [data getBytes:&scratchVal length:data.length];
    UInt16 qz_read;
    qz_read = (scratchVal[3] & 0xff) | ((scratchVal[2] << 8) & 0xff00);
    float qz = (float)qz_read/QUATERNION_RANGE;
    if(qz >=2) qz-=4;
    return qz;
}
...
-(float) calcGyroX:(NSData *)data {
    ...
    [data getBytes:&scratchVal length:6];
    int16_t rawX = (scratchVal[0] & 0xff) | ((scratchVal[1] << 8) & 0xff00);
    self.lastX = (((float)rawX * 1.0) / GYRO_RANGE) * -1;
    return (self.lastX - self.calX);
    //return (((float)rawX * 1.0) / GYRO_RANGE) * -1;
}
-(float) calcGyroY:(NSData *)data {
    ...
    [data getBytes:&scratchVal length:6];
    int16_t rawY = ((scratchVal[2] & 0xff) | ((scratchVal[3] << 8) & 0xff00));
    self.lastY = (((float)rawY * 1.0) / GYRO_RANGE) * -1;
    return (self.lastY - self.calY);
    //return (((float)rawY * 1.0) / GYRO_RANGE) * -1;
}
-(float) calcGyroZ:(NSData *)data {
    char scratchVal[6];
    [data getBytes:&scratchVal length:6];
    int16_t rawZ = (scratchVal[4] & 0xff) | ((scratchVal[5] << 8) & 0xff00);
    self.lastZ = ((float)rawZ * 1.0) / GYRO_RANGE;
    return (self.lastZ - self.calZ);
    //return ((float)rawZ * 1.0) / GYRO_RANGE;
}
...
-(float) calcAccX:(NSData *)data {
    ...
    char scratchVal[6];
    [data getBytes:&scratchVal length:6];
    int16_t rawX = (scratchVal[1] & 0xff) | ((scratchVal[0] << 8) & 0xff00);
    return (float)rawX/ACC_RANGE;
}
-(float) calcAccY:(NSData *)data {
    //Orientation of sensor on board means we need to swap Y (multiplying with -1)
    char scratchVal[6];
    [data getBytes:&scratchVal length:6];
    int16_t rawY = ((scratchVal[3] & 0xff) | ((scratchVal[2] << 8) & 0xff00));
    return (float)rawY/ACC_RANGE;
}
-(float) calcAccZ:(NSData *)data {
    char scratchVal[6];
    [data getBytes:&scratchVal length:6];
    int16_t rawZ = (scratchVal[5] & 0xff) | ((scratchVal[4] << 8) & 0xff00);
    return (float)rawZ/ACC_RANGE;
}
...
+(float) calcQx:(NSData *)data {
    ...
    [data getBytes:&scratchVal length:data.length];
    UInt16 qx_read;
    qx_read = (scratchVal[1] & 0xff) | ((scratchVal[0] << 8) & 0xff00);
    float qx = (float)qx_read/QUATERNION_RANGE;
    if(qx >=2) qx-=4;
    return qx;
}
+(float) calcQw:(NSData *)data {
    ...
    [data getBytes:&scratchVal length:data.length];
    UInt16 qw_read;
    qw_read = (scratchVal[3] & 0xff) | ((scratchVal[2] << 8) & 0xff00);
    float qw = (float)qw_read/QUATERNION_RANGE;
    if(qw >=2) qw-=4;
    return qw;
